...
function solution(n, lost, reserve) {
  ...
  // 도난당한 학생이 체육복을 빌릴 수 있도록 체크
  for (let i = 0; i < filteredLost.length; i++) {
    const student = filteredLost[i];
    const borrowIndex = filteredReserve.findIndex(
      (r) => Math.abs(r - student) === 1
    );
    if (borrowIndex !== -1) {
      answer++;
      filteredReserve.splice(borrowIndex, 1); // 빌려준 학생 제거
    }
  }
